...
//         向DL_QUEUE 发送消息  10*1000毫秒后过期 形成死信
    ...
    /**
     * 测试死信队列. for  RabbitTTLAndDLXMyselfConfig
     *
     * @param p the p
     * @return the response entity
     */
    @RequestMapping("/dead/test2")
    public ResponseEntity deadLetter2(String p) {
        CorrelationData correlationData = new CorrelationData(UUID.randomUUID().toString());
//        声明消息处理器  这个对消息进行处理  可以设置一些参数   对消息进行一些定制化处理   我们这里  来设置消息的编码  以及消息的过期时间  因为在.net 以及其他版本过期时间不一致   这里的时间毫秒值 为字符串
        MessagePostProcessor messagePostProcessor = message -> {
            MessageProperties messageProperties = message.getMessageProperties();
...
//         向DL_QUEUE 发送消息  10*1000毫秒后过期 形成死信
        rabbitTemplate.convertAndSend("EMAIL_EXCHANGE", "EMAIL_ROUTING_KEY", p, messagePostProcessor, correlationData);
        return ResponseEntity.ok();
    }
}
